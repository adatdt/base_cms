...
const antiXssRegex = /^[^<>'";()[\]\\/]*$/;
const xssErrorMessage = "Input mengandung karakter khusus yang dilarang untuk alasan keamanan";
...
const urlErrorMessage = "Format Base URL tidak valid (Harus diawali http:// atau https://)";
...
export const providerAddFormSchema = z.object({
    ...
    file_provider: z
        ...
        .refine((file) => file?.size <= MAX_FILE_SIZE, `Ukuran berkas maksimal adalah 3.88MB.`)
        .refine((file) => {
    ...
    // Base URL Non-SNAP (Variant: text)
    // 💡 PERBAIKAN SONARQUBE: Mengganti .url() dengan .refine() berbasis native URL parser
    non_snap: z
        .string()
        .min(1, { message: "Base URL Non-SNAP wajib diisi" })
        .refine(isValidSecureUrl, { message: `Format Base URL Non-SNAP tidak valid (Harus diawali http:// atau https://)` }),

    // Base URL SNAP (Variant: text)
    snap: z
        .string()
        .min(1, { message: "Base URL SNAP wajib diisi" })
        .refine(isValidSecureUrl, { message: `Format Base URL SNAP tidak valid (Harus diawali http:// atau https://)` }),

    // Config Timeout Read (Variant: text-addon)
    timeout_read: z
        .string()
        .min(1, { message: "Config timeout read wajib diisi" })
        .regex(/^\d+$/, { message: "Timeout harus berupa angka murni (detik)" }),
...
export const providerEditFormSchema = providerAddFormSchema.extend({
    ...
    // Overwrite 'file_provider' pada mode edit agar bisa menerima file lama (string URL) atau File baru (opsional)
    file_provider: z
        .any()
        .refine((file) => file !== undefined && file !== null && file !== "", "File wajib disertakan.")
        .refine(
